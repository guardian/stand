...
import {
	...
	defaultMenuPopoverTheme,
	defaultMenuSectionTheme,
	defaultMenuSeparatorTheme,
	defaultMenuTheme,
	menuItemAsideStyles,
...
export function Menu({
	...
	// Iterate through the children of Menu and separate out the MenuToggle (trigger) and the MenuItem/MenuSection/MenuSeparator (menu) components.
	// We also clone the MenuItem, MenuSection, MenuSeparator components to pass down the size prop, and add a key for React's reconciliation.
	// Only MenuToggle, MenuItem, MenuSection, and MenuSeparator components are allowed as direct children of Menu.
	React.Children.forEach(children, (child, i) => {
		if (!React.isValidElement(child)) {
			return;
		}
		...
		if (child.type === MenuToggle) {
			...
			menu.push(
				React.cloneElement(child as React.ReactElement<MenuSeparatorProps>, {
					key: `${child.key}-${i}`,
				}),
			);
		}
