...
<Meta of={AvatarLinkStories} />

# AvatarLink

An anchor element styled like the Stand `Avatar` component, and based on the react-aria `Link` component. It displays only a user's profile image or initials in a circular container and keeps link semantics (including `href`, `target`, and `rel`) while sharing the same color variants and sizes as `Avatar`. Refer to React Aria's [Link docs](https://react-aria.adobe.com/Link) for usage guidance.
...
```tsx
import { AvatarLink } from '@guardian/stand/Link';

/* types, if required */
import type { AvatarLinkProps, AvatarLinkTheme } from '@guardian/stand/avatarLink';

/* Initials only */
<AvatarLink initials="AB" size="md" href="/user" />
...
The props are an intersection of the `Avatar` and `ReactAria Link` props.
...
We recommend using the AvatarLink component as provided, but it can be customised using the `theme` or `cssOverrides` props as required.

### Custom theme

The `theme` prop allows you to override specific design tokens for the AvatarLink component:

<Canvas layout="padded" of={CustomTheme} />

```tsx
import type { AvatarLinkTheme } from '@guardian/stand/avatarLink';
import { AvatarLink } from '@guardian/stand/avatarLink';
...
const Component = () => (
	<AvatarLink initials="CO" size="md" color="red" cssOverrides={customStyles} />
);
```

## Custom Component Build

If you're not using React/Emotion, or `@guardian/stand` is not compatible with your project, you can create a custom AvatarLink component using the styles defined in the `AvatarLinkTheme` type.
...
You can import the AvatarLink styles as CSS from the package, make sure that your build process supports importing CSS from `node_modules`/`package.json`:
