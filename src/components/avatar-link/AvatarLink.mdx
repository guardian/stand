...
An anchor element styled like the Stand `Avatar` component, and based on the react-aria `Link` component. It displays only an icon and keeps link semantics (including `href`, `target`, and `rel`) while sharing the same color variants and sizes as `Avatar`. Refer to React Aria's [Link docs](https://react-aria.adobe.com/Link) for usage guidance.
