/**
 * InlineMessage component entry point
 *
 * Peer dependencies required to use these components:
 * - `@emotion/react`
 * - `react`
 * - `react-dom`
 * - `typescript`
 *
 * See the `peerDependencies` section of package.json for compatible versions.
 *
 * If you only need the built CSS (./component/inlineMessage.css),
 * you don't need to install these.
 */
export { InlineMessage } from './components/InlineMessage/InlineMessage';
export type { InlineMessageProps } from './components/InlineMessage/types';
export type { PartialInlineMessageTheme as InlineMessageTheme } from './components/InlineMessage/styles';
export { componentInlineMessage } from './styleD/build/typescript/component/inlineMessage';
export type { ComponentInlineMessage } from './styleD/build/typescript/component/inlineMessage';
