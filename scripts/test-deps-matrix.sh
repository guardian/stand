...
USER_RAC_VERSIONS=${RAC_VERSIONS-}
...
if [ -f "$VERSIONS_JSON_FILE" ]; then
	...
	RAC_VERSIONS=$(jq -r '.["react-aria-components"] | join(" ")' "$VERSIONS_JSON_FILE")
    EXCLUDE_RULES=()
...
[ -n "${USER_RAC_VERSIONS}" ] && RAC_VERSIONS="$USER_RAC_VERSIONS"
...
if [ -z "${RAC_VERSIONS:-}" ]; then missing+=("RAC_VERSIONS"); fi
...
echo "React Aria Components versions: $RAC_VERSIONS"
	...
	for rule in "${EXCLUDE_RULES[@]:-}"; do
		...
		for kv in $rule; do
			...
			case "$key" in
				...
				react-aria-components) [ "$racv" = "$val" ] || match=0 ;;
			esac
			[ $match -eq 0 ] && break
		done
		[ $match -eq 1 ] && return 0
	...
	for ev in $EMOTION_VERSIONS; do
		for tv in $TS_VERSIONS; do
			...
			if should_skip "$rv" "$ev" "$tv" "$racv"; then
				printf '\n-- Skipping excluded combination React %s | Emotion %s | TypeScript %s | react-aria-components %s\n' "$rv" "$ev" "$tv" "$racv"
				# Use 'skip' for all statuses if excluded
				RESULT_ROWS+=("$REACT_MAJOR_VERSION|$ev|$tv|$racv|skip|skip|skip|skip")
				continue
			fi
			...
			printf '\n=============================================================\n'
			echo "Testing React $REACT_MAJOR_VERSION  |  @emotion/react $ev  |  TypeScript $tv  |  react-aria-components $racv"
			echo "============================================================="
			...
			if [ "$PLAYWRIGHT_CT_REACT_18" = true ]; then
				...
				pnpm add -D @playwright/experimental-ct-react@"^1.60.0"
			else
				echo "-- Using existing @playwright/experimental-ct-react17 for React $REACT_MAJOR_VERSION"
				pnpm add -D @playwright/experimental-ct-react17@"^1.60.0"
			fi
			...
			RESULT_ROWS+=("$REACT_MAJOR_VERSION|$ev|$tv|$racv|$TYPECHECK_STATUS|$UNIT_TEST_STATUS|$E2E_TEST_STATUS|$BUILD_STATUS")
			done
		done
	done
done
...
if [ -n "$OUTPUT_FILE" ]; then
	: > "$OUTPUT_FILE"
	echo "| React | Emotion | TypeScript | RAC | Typecheck | Unit | E2E | Build |" | tee "$OUTPUT_FILE"
	echo "|-------|---------|------------|-----|-----------|------|-----|-------|" | tee -a "$OUTPUT_FILE"
else
	echo "| React | Emotion | TypeScript | RAC | Typecheck | Unit | E2E | Build |"
	echo "|-------|---------|------------|-----|-----------|------|-----|-------|"
fi
OVERALL_FAIL=0
for row in "${RESULT_ROWS[@]}"; do
	...
	if [ -n "$OUTPUT_FILE" ]; then
		printf '| %s | %s | %s | %s | %s | %s | %s | %s |\n' "$rv" "$ev" "$tv" "$rac" "$type" "$unit" "$e2e" "$build" | tee -a "$OUTPUT_FILE"
	else
		printf '| %s | %s | %s | %s | %s | %s | %s | %s |\n' "$rv" "$ev" "$tv" "$rac" "$type" "$unit" "$e2e" "$build"
	fi
done
