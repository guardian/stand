...
We are confident Stand users will want to customise Stand components to some degree. We would like to provide APIs that enable consumers to customise stand components, encouraging adoption, without giving them a surface area that proves an obstacle to library updates.
...
2.) is a great way of facilitating common and known styling needs, especially when they scale across a codebase (e.g. general colour or spacing requirements.)

3.) is very useful when applying ad-hoc styling, especially when layout decisions require targeting specific components — for example, adding \`margin-left: auto\` to ensure that a particular element is right-aligned in a flex context without resorting to additional wrapping markup.

Colloquial usage in other design systems within the Guardian, e.g. EUI in Typerighter [(code search for `css=`)](https://github.com/search?q=repo%3Aguardian%2Ftyperighter%20%22css%3D%22&type=code), gives us confidence that providing this API will add significant convenience value, which we think is important for adoption. We’re aware that exposing an arbitrary surface area for styling gives consumer an opportunity to expose themselves to breaking changes that are not reflected in semver.

Unlike in source, we think the less stringent styling requirements in our tooling (as opposed to audience-facing products) will lead to fewer instances of deeply-coupled aesthetic styling being applied in this way, and so we are happy to optimise for convenience at the expense of the increase in surface area.
